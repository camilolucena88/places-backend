...
// under demo paths
const demoPath = rootPath + '/places-backend';

const assetDistPath = demoPath + '/static';
...
function getEntryFiles() {
  ...
  // Keen css pages (single page use)
  (glob.sync(path.relative('./', demoPath + '/src') + '/sass/pages/**/!(_)*.scss') || []).forEach(file => {
    file = './'+file
    entries[file.replace(/.*sass\/(.*?)\.scss$/ig, 'css/$1')] = file;
  });
  (glob.sync(path.relative('./', demoPath + '/src') + '/js/pages/**/!(_)*.js') || []).forEach(file => {
    file = './'+file
    entries[file.replace(/.*js\/(.*?)\.js$/ig, 'js/$1')] = file;
  });

  // Keen theme
  (glob.sync(path.relative('./', demoPath + '/src') + '/sass/themes/**/!(_)*.scss') || []).forEach(file => {
    file = './'+file
    entries[file.replace(/.*sass\/(.*?)\.scss$/ig, 'css/$1')] = file;
  });
